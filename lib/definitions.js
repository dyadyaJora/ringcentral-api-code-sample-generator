/*
    ...
    description:        Utility Module for handling API response formats (i.e. definitions)
*/
...
function generateFullDefinition(responseType, isV3) {
    ...
    if (isV3) {
        definition = spec.components.schemas[responseType]
    } else {
    ...
    for (key in definition.properties) {
        ...
        const schema = ref.substring(ref.lastIndexOf('/') + 1)
        let subdef = generateFullDefinition(schema, isV3)
