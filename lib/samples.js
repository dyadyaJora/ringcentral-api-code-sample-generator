...
function dirPathTo(operation, root = process.env.CODE_SAMPLES_OUTPUT) {
    ...
    if (!!operation || !!operation.tags || !!operation.tags.length) {
        return path.resolve(`${root}${esc(operation.summary)}/code-samples/`);
    }

    tag = operation.tags[0];
    return path.resolve(`${root}${esc(xtaggroup[tag])}/${esc(tag)}/${esc(operation.summary)}/code-samples/`)
}
...
function filePathToCurl(data, suffix) {
    return path.resolve(`${dirPathTo(data.operation)}/${data.operation.operationId}.${suffix}`)
}
